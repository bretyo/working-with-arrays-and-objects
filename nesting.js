...
function employeeUpdater(){
  for (let index = 0; index < employees.length; index++) {
    employees[index].firstName === "Theo"? delete employees[index] 
    : employees[index].firstName === "Lorie"? employees[index].department = "HR":
    null;
...
function removeDuplicates(){
  let arr = [""];
  let dupe = false;
  for (let index = 0; index < workplaceAccidents.length; index++) {
    for (let j = 0; j < arr.length; j++) {
      if(workplaceAccidents[index] === arr[j] ) dupe = true;
      
    }
    dupe? !dupe : arr.push(workplaceAccidents[index])
  }
  return arr;
}
